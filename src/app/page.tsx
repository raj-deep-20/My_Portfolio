...
import { 
  ...
  BookOpen, Code2, Award, FileText, Cpu, 
  Globe, Database, Camera,
  Toolbox,
  CodeIcon,
  Pen,
  BalloonIcon
...
const Navbar = () => {
  ...
  const navLinks = [
    ...
    { label: "Extra Curriculars", id: "extra-curriculars" },
    ...
    <nav className="fixed w-full z-50 bg-[#0B0E14]/90 backdrop-blur-md border-b border-slate-800">
      <div className="container mx-auto px-6 h-16 flex justify-between items-center">
        ...
        <div className="hidden md:flex gap-8 text-xs font-mono uppercase tracking-widest">
          {navLinks.map((link) => (
            <a key={link.id} href={`#${link.id}`} className="hover:text-cyan-400 transition-colors">
              {link.label}
            </a>
          ))}
        </div>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="md:hidden bg-[#112240] border-b border-slate-800 absolute w-full px-6 py-8 flex flex-col gap-4">
            {navLinks.map((link) => (
              <a key={link.id} href={`#${link.id}`} onClick={() => setIsOpen(false)} className="text-lg font-semibold border-b border-slate-700 pb-2">{link.label}</a>
            ))}
          </motion.div>
        )}
      </AnimatePresence>
...
export default function Portfolio() {
  return (
    <div className="bg-[#0B0E14] text-slate-300 min-h-screen overflow-x-hidden">
      <Navbar />

      {/* Hero Section */}
      <section id="about" className="pt-40 pb-20 container mx-auto px-6 flex flex-col md:flex-row items-center gap-16">
        <div className="flex-1 order-2 md:order-1">
          <motion.h1 initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.55, ease: "easeOut" }} className="text-5xl md:text-7xl font-bold text-white mb-4">
            Rajdeep Podder
          </motion.h1>
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2, duration: 0.4 }} className="text-xl md:text-4xl font-mono mb-6 h-8 md:h-12">
        ...
        <div className="relative group order-1 md:order-2">
          <div className="w-64 h-64 md:w-80 md:h-80 border-2 border-cyan-400 rounded-3xl p-3 rotate-3 mx-auto md:mx-10 group-hover:rotate-0 transition-all duration-500">
      ...
      {/* Tech Stack */}
      <section id="tech-stack" className="py-24 bg-[#0d1117]">
      ...
      {/* Academic Records */}
      <section id="academics" className="py-24 container mx-auto px-6">
        ...
        >
          {[
            ...
            <motion.div key={i} variants={fadeUp} className="bg-[#112240] p-8 rounded-2xl relative overflow-hidden group">
        ...
        <div className="container mx-auto px-6">
          ...
          >
            {[
              ...
              <motion.div key={i} variants={fadeUp} className="bg-[#112240] rounded-2xl overflow-hidden border border-slate-800 flex flex-col">
      ...
      <section id="extra-curriculars" className="py-24 container mx-auto px-6">
